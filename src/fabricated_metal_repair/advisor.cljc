(ns fabricated-metal-repair.advisor
  "Repair Advisor client -- the *contained intelligence node* for the
  fabricated-metal-repair OPERATIONS COORDINATION actor.

  It normalizes repair-record logging (diagnostic-finding/repair-work-
  performed/parts-used data for structural steel members, storage tanks,
  boilers/pressure vessels and metal furniture), drafts a
  diagnostic/repair/testing SCHEDULE PROPOSAL (never a repair-equipment/
  welding-tool control command or a return-to-service sign-off), drafts a
  safety-concern FLAG (structural-integrity failure, weld-repair-quality
  defect, pressure-test failure), and drafts a replacement-parts/welding-
  consumables SUPPLY-ORDER PROPOSAL. CRITICAL: it is a smart-but-untrusted
  advisor, and it is SCOPED -- it holds NO repair-equipment/welding-tool
  control authority and NO return-to-service sign-off authority (that is
  the licensed repair technician's/inspector's exclusively). It returns a
  *proposal* (with a rationale + the fields it cited), NEVER a committed
  ...
  return-to-service authorization. Every output carries `:effect
  :propose` and is censored downstream by `fabricated-metal-repair.
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     :propose       ; ALWAYS :propose -- see `fabricated-metal-repair.governor`
...
(defn- schedule-repair-operation
  ...
  operation window (never a repair-equipment/welding-tool control command
  or a return-to-service sign-off; see `fabricated-metal-repair.governor`
  ns docstring). `:stake :schedule-repair-operation`."
...
(defn- flag-safety-concern
  ...
  failure, weld-repair-quality defect or pressure-test failure concern
  for human review. `:stake :flag-safety-concern` -- ALWAYS escalates to
  a human, unconditionally, regardless of confidence (see README
  `Actuation` + `fabricated-metal-repair.governor` ns docstring
  `high-stakes`)."
...
(defn- order-supplies
  "Draft a replacement-parts/welding-consumables SUPPLY-ORDER PROPOSAL.
  `:stake :order-supplies` -- escalates when `:cost-usd` exceeds
  `fabricated-metal-repair.governor/supply-order-cost-threshold-usd`, or
  when confidence is low (see `fabricated-metal-repair.governor`)."
  [db {:keys [subject items cost-usd vendor]}]
  (let [a (store/equipment db subject)]
    {:summary    (str subject " 向け交換部品/溶接消耗品発注提案 (" (pr-str items) ", "
...
(def ^:private system-prompt
  (str "あなたは金属製品修理工房の運行調整（オペレーションズ・コーディネーション）"
       ...
       "重要: あなたは修理設備/溶接ツールを直接操作するコマンドや、返品・稼働再開"
       "（return-to-service）の確定を伴う提案を絶対に作成してはいけません（免許を"
       "持つ修理技術者/検査員の専権事項）。登録されていない法域の要件を絶対に"
       "創作してはいけません。legal-basisが無い場合は:citesを空にしconfidenceを"
       "上げないこと。"))
