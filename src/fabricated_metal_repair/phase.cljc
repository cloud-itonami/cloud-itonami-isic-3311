(ns fabricated-metal-repair.phase
  "Phase 0->3 staged rollout -- the fabricated-metal-repair-coordination
  analog of `installation.phase`/`demolition.phase`/`electrical-
  equipment-repair.phase`/`other-equipment-repair.phase`.
  ...
  produce carries `:effect :propose` (see `fabricated-metal-repair.
  ...
  equipment/welding tools, never signs off on return-to-service -- that
  authority is the licensed repair technician's/inspector's exclusively.
  ...
  failure, weld-repair-quality defect or pressure-test failure is exactly
  the judgment this actor must never let auto-commit; it is always a
  human's call. `fabricated-metal-repair.governor`'s `high-stakes` set
  enforces the same invariant independently -- two layers, not one, agree
  on this (see `fabricated-metal-repair.governor` ns docstring).
  ...
  phase 3's `:auto` set here -- the SAME shape `cloud-itonami-isic-3314`/
  `cloud-itonami-isic-3319` use. `fabricated-metal-repair.governor`'s own
  equipment-verification / legal-basis / unresolved-concern HARD checks
  (checks 4-6) PLUS its `:return-to-service-sign-off?` forbidden-action-
  class block (check 3) already gate the actual structural/pressure-
  integrity hazard surface independently of phase -- the schedule op
  itself is still only ever a proposed diagnostic/repair/testing WINDOW,
  never a live-work authorization. This is a deliberate design choice
  matching this fleet's established shape, not an oversight -- see
  `fabricated-metal-repair.governor` ns docstring `high-stakes`.
  ...
  supplies` BELOW the cost threshold (see `fabricated-metal-repair.
  ...
  layering `installation.phase`/`construction.phase`/`electrical-
  equipment-repair.phase`/`other-equipment-repair.phase` established for
  cost-scoped ops.")
