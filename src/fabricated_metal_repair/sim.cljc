(ns fabricated-metal-repair.sim
...
(defn -main [& _]
  (let [db (store/seed-db)
    ...
    (println (exec! actor "t1" {:op :log-repair-record :subject "equipment-1"
                                :patch {:id "equipment-1" :diagnostic-notes "hairline crack found along the tank's longitudinal seam weld during ultrasonic scan"}} operator))
    ...
    (let [r (exec! actor "t3" {:op :flag-safety-concern :subject "equipment-1"
                               :concern-type :pressure-test-failure
                               :concern-description "耐圧試験で規定圧力に達する前に微小漏洩を検知、溶接補修範囲の再検討が必要。"} operator)]
      (println r)
    ...
    (println (exec! actor "t6" {:op :order-supplies :subject "equipment-1"
                                :items ["welding-rod-e7018" "steel-plate-patch"]
                                :cost-usd 650 :vendor "Local Steel Fabrication Supply Co."} operator))
