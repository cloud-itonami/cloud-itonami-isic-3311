(ns fabricated-metal-repair.governor
  "Repair Governor -- the independent compliance layer named in
  `blueprint.edn` (`:itonami.blueprint/governor :fabricated-metal-repair-
  governor`) that earns the Repair Advisor the right to commit. The LLM
  has no notion of fabricated-metal-repair-shop safety law, whether an
  equipment/work-order's own recorded verification actually reflects
  reality, or when a proposal has quietly drifted outside this actor's
  charter into repair-equipment/welding-tool control or a
  return-to-service sign-off, so this MUST be a separate system able to
  *reject* a proposal and fall back to HOLD -- the fabricated-metal-
  repair analog of `cloud-itonami-isic-3314`'s Repair Governor and
  `cloud-itonami-isic-3319`'s Repair Governor.

  ## Scope -- this is a COORDINATION-ONLY actor

  This actor NEVER controls repair equipment/welding tools and NEVER
  signs off on return-to-service -- that authority is the licensed
  repair technician's/inspector's EXCLUSIVELY. Every proposal this
  actor's Repair Advisor can produce carries `:effect :propose` and
  NOTHING else -- committing a proposal here means 'this coordination
  artifact is now logged/scheduled/flagged/ordered', never 'a welding
  tool was operated' or 'equipment is authorized to return to service'.
  ...
  5-6, which are ordinary per-jurisdiction/ground-truth safety gates.
  ...
  CANNOT override them.
    ...
    5. Legal-basis missing       -- for `:schedule-repair-operation`, did
                                     ...
                                     (`fabricated-metal-repair.facts`), or
    ...
    6. Unresolved safety concern -- for `:schedule-repair-operation`, the
                                     ...
                                     `high-stakes` below and `fabricated-
                                     metal-repair.phase` ns docstring).
  ...
  jurisdictions), this actor's `fabricated-metal-repair.facts` catalog
  found NO `:quantitative` jurisdiction for the post-repair inspection-
  before-return-to-service duty (see that ns docstring) -- so there is no
  numeric bright line to independently re-check here, and no seventh HARD
  check exists for it. This is an honest absence, not an oversight.
  ...
  the human may approve.
    ...
    - `:flag-safety-concern` is UNCONDITIONALLY a member of `high-stakes`
      ...
      integrity failure, weld-repair-quality defect or pressure-test
      failure is exactly the kind of judgment this actor must never let
      auto-commit.
    - `:schedule-repair-operation` is DELIBERATELY NOT a permanent member
      ...
      scheduling a diagnostic/repair/testing window for a fabricated-
      metal item (structural steel, tank, boiler, metal furniture) MAY
      auto-commit at phase 3 when the governor is completely clean (see
      `fabricated-metal-repair.phase` ns docstring 'Actuation' section)
      -- the SAME escalation shape `cloud-itonami-isic-3314`/`cloud-
      itonami-isic-3319` use, because checks 3-6 above (forbidden-action-
      class including `:return-to-service-sign-off?`, equipment-
      verification, legal-basis, unresolved-concern) already gate the
      real hazard surface independently of phase -- the schedule op
      itself is still only ever a proposed WINDOW, never a live-work
      authorization.
  ...
  (:require [fabricated-metal-repair.store :as store]))
...
(def high-stakes
  ...
  surface stays aligned with `cloud-itonami-isic-3314`/`cloud-itonami-
  isic-3319`. `:order-supplies` is deliberately NOT a permanent member
  either -- its escalation is a SOFT, cost-scoped rule computed in
  `check` below, not a blanket 'always a human' rule."
  #{:flag-safety-concern})
...
(defn- forbidden-action-class-violations
  ...
  welding-tool-control / direct-actuation / return-to-service-sign-off
  ...
  (let [v (:value proposal)]
    (when (and (map? v)
               (or (true? (:repair-equipment-control? v))
                   (true? (:welding-tool-control? v))
                   (true? (:direct-actuation? v))
                   (true? (:return-to-service-sign-off? v))))
      [{:rule :forbidden-action-class
        :detail "修理設備/溶接ツールの直接操作コマンド、または返品・稼働再開（return-to-service）の確定を伴う提案は恒久的に禁止（免許を持つ修理技術者/検査員の専権事項）"}])))
...
(defn- legal-basis-missing-violations
  ...
  source -- never invent a jurisdiction's post-repair inspection-before-
  return-to-service requirements. See ns docstring check 5."
  [{:keys [op]} proposal]
  (when (= op :schedule-repair-operation)
    (let [value (:value proposal)]
      (when (or (empty? (:cites proposal))
        ...
        [{:rule :no-legal-basis
          :detail "公式legal-basisの引用が無い提案は金属製品修理作業スケジュール提案として扱えない"}]))))
    ...
    (let [a (store/equipment st subject)]
      (when (true? (:safety-concern-unresolved? a))
        [{:rule :unresolved-safety-concern
          :detail (str subject " は未解決の安全性懸念（構造健全性/溶接補修品質/耐圧試験不合格）がある状態での金属製品修理作業スケジュール提案")}]))))
