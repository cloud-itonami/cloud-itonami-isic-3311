(ns fabricated-metal-repair.store
  "SSoT for the fabricated-metal-repair-coordination actor, behind a
  ...
  (test/fabricated_metal_repair/store_contract_test.clj), which is the
  ...
  number of times for the same equipment/work-order (a structural steel
  member, storage tank, boiler/pressure vessel or metal-furniture unit
  ...
  welding tools or signs off on return-to-service -- it only ever
  proposes, logs and schedules (`:effect :propose` unconditionally, see
  `fabricated-metal-repair.governor` ns docstring). The equipment/work-
  ...
  immutable log."
  (:require [fabricated-metal-repair.registry :as registry]
...
(defprotocol Store
  ...
  (all-equipment [s])
  (ledger [s])
  (repair-record-log-history [s] "the append-only repair-record-log history (fabricated-metal-repair.registry drafts)")
...
(defn demo-data
  ...
  path (equipment-1, JPN storage tank), the uncovered-jurisdiction / not-
  verified / unresolved-safety-concern failure modes (equipment-2..
  equipment-4), a cross-jurisdiction (USA) happy path (equipment-5), and
  the honestly-qualitative (DEU/EU) jurisdiction that never fabricates a
  numeric lead-time (equipment-6) -- so the actor + tests run offline."
  []
  {:equipment
   {"equipment-1" {:id "equipment-1" :name "Kōji Steel Fabrication -- Storage Tank Structural Weld Repair (work order #5521)"
                   :jurisdiction "JPN" :equipment-verified? true
                   :safety-concern-unresolved? false
                   :safety-contacts [{:name "Kōji (repair technician)" :email "koji@example.com" :phone "+819000000031"}
                                     {:name "Aiko (shop safety officer)" :email "aiko@example.com" :phone "+819000000032"}]
                   :status :ready-to-schedule}
    "equipment-2" {:id "equipment-2" :name "Atlantis Structural Works -- Boiler Shell Weld Repair"
                   :jurisdiction "ATL" :equipment-verified? true
                   :safety-concern-unresolved? false
                   :safety-contacts []
                   :status :intake}
    "equipment-3" {:id "equipment-3" :name "山田製缶工房 圧力容器オーバーホール"
                   :jurisdiction "JPN" :equipment-verified? false
                   :safety-concern-unresolved? false
                   :safety-contacts [{:name "Yamada (repair technician)" :email "yamada@example.com" :phone "+819000000033"}]
                   :status :unverified}
    "equipment-4" {:id "equipment-4" :name "ことぶき鉄構サービス 構造用鋼材フレーム修理"
                   :jurisdiction "JPN" :equipment-verified? true
                   :safety-concern-unresolved? true
                   :safety-contacts [{:name "Watanabe (repair technician)" :email "watanabe@example.com" :phone "+819000000034"}
                                     {:name "Yamamoto (shop safety officer)" :email "yamamoto@example.com" :phone "+819000000035"}]
                   :status :concern-open}
    "equipment-5" {:id "equipment-5" :name "Riverside Municipal Utility -- Boiler Pressure Vessel Repair"
                   :jurisdiction "USA" :equipment-verified? true
                   :safety-concern-unresolved? false
                   :safety-contacts [{:name "Casey (repair technician)" :email "casey@example.com" :phone "+15550000031"}]
                   :status :ready-to-schedule}
    "equipment-6" {:id "equipment-6" :name "Alt-Stahlbau-Werkstatt -- Drucktank-Reparatur"
                   :jurisdiction "DEU" :equipment-verified? true
                   :safety-concern-unresolved? false
                   :safety-contacts [{:name "Müller (repair technician)" :email "mueller@example.com" :phone "+4915000000031"}]
                   :status :ready-to-schedule}}})
