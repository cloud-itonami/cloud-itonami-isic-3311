...
`cloud-itonami-isic-3311` accepts contributions to the OSS blueprint,
...
- Never extend this actor's op-allowlist, advisor or governor to permit
  repair-equipment/welding-tool control or return-to-service sign-off
...
- Extending `fabricated-metal-repair.facts` coverage to a new
  jurisdiction requires a real, citable official source -- never
...
- Do not add JVM-only interop to `src/` (no `java.net.http`, no other
  ...
  added, must go behind `fabricated-metal-repair.notify/Notifier` via a
