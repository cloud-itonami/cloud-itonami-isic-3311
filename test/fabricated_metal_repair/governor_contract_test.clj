(ns fabricated-metal-repair.governor-contract-test
  "The governor contract as executable tests -- the fabricated-metal-
  repair-coordination analog of `installation.governor-contract-test`/
  `electrical-equipment-repair.governor-contract-test`/`other-equipment-
  repair.governor-contract-test`. The single invariant under test:
  ...
  (:require [clojure.test :refer [deftest is testing]]
            ...
            [fabricated-metal-repair.governor :as governor]
            [fabricated-metal-repair.phase :as phase]))
...
(deftest clean-log-repair-record-auto-commits
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-repair-record :subject "equipment-1" :patch {:id "equipment-1" :diagnostic-notes "loose bolted flange gasket"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "loose bolted flange gasket" (:diagnostic-notes (store/equipment db "equipment-1"))) "SSoT actually updated")
...
(deftest fabricated-jurisdiction-is-held
  (testing "equipment-2 (ATL, no spec-basis in fabricated-metal-repair.facts) -> HOLD, never reaches a human"
    (let [[db actor] (fresh)
  ...
  (testing "equipment-1 is fully clean -- :flag-safety-concern STILL always interrupts, unconditionally"
    (let [[db actor] (fresh)
          r1 (exec-op actor "t10" {:op :flag-safety-concern :subject "equipment-1"
                                   ...
                                   :concern-description "hairline crack found along the tank's longitudinal seam weld"} operator)]
...
(deftest flag-safety-concern-triggers-notification-only-after-approval
  (let [[_db actor] (fresh)
        r1 (exec-op actor "t11" {:op :flag-safety-concern :subject "equipment-1"
                                 :concern-type :pressure-test-failure :concern-description "minor leak detected before rated pressure during hydrostatic test"} operator)]
...
(deftest order-supplies-below-threshold-auto-commits
  (let [[db actor] (fresh)
        res (exec-op actor "t12" {:op :order-supplies :subject "equipment-1"
                                  :items ["welding-rod-e7018"] :cost-usd 300 :vendor "Local Steel Fabrication Supply Co."} operator)]
    (is (= :commit (get-in res [:state :disposition])))
...
(deftest order-supplies-above-threshold-escalates
  (let [[db actor] (fresh)
        r1 (exec-op actor "t13" {:op :order-supplies :subject "equipment-1"
                                 :items ["replacement-tank-shell-section"] :cost-usd 9000 :vendor "Heavy Steel Fabrication Supply"} operator)]
...
(deftest forbidden-action-class-markers-are-permanent-hard-violations
  (doseq [marker [:repair-equipment-control? :welding-tool-control? :direct-actuation?
...
(deftest schedule-repair-operation-is-auto-eligible-at-phase-3-unlike-3320
  (testing "structural invariant: this actor's escalation surface auto-commits a governor-clean schedule at phase 3, the same shape cloud-itonami-isic-3314/cloud-itonami-isic-3319 use"
