(ns fabricated-metal-repair.registry-test
  (:require [clojure.test :refer [deftest is]]
            [fabricated-metal-repair.registry :as r]))
...
(def sample-equipment
  {:id "equipment-1" :name "Kōji Steel Fabrication -- Storage Tank Structural Weld Repair (work order #5521)" :jurisdiction "JPN"})

(deftest safety-concern-notice-cites-legal-basis-inline
  (let [doc (r/render-safety-concern-notice sample-equipment "JPN-SCF-000000" "縦継手溶接部にヘアラインクラックを検出")]
    (is (re-find #"JPN-SCF-000000" doc))
    (is (re-find #"ボイラー及び圧力容器安全規則" doc))
